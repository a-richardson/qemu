...
#if USER_TRACE_DEBUG
// #define user_trace_dbg(...) qemu_log_mask(CPU_LOG_INSTR, "=== " __VA_ARGS__)
...
void helper_instr_start(CPUMIPSState *env)
{
    ...
    user_trace_dbg("Switching on tracing @ 0x%llx ASID %d\n",
        (unsigned long long)env->active_tc.PC, env->CP0_EntryHi & 0xFF);
    ...
    if (env->user_only_tracing_enabled && !IN_USERSPACE(env)) {
        user_trace_dbg("Delaying tracing request at 0x%llx "
...
{
    user_trace_dbg("Switching off tracing @ 0x%llx ASID %d\n",
        (unsigned long long)env->active_tc.PC, env->CP0_EntryHi & 0xFF);
    qemu_set_log(qemu_loglevel & ~CHERI_DEFAULT_TRACE_FLAG);
...
{
    user_trace_dbg("User-mode only tracing enabled at 0x%llx, ASID %d\n",
        (unsigned long long)env->active_tc.PC, env->CP0_EntryHi & 0xFF);
...
void helper_instr_stop_user_mode_only(CPUMIPSState *env)
{
    ...
    if (env->tracing_suspended) {
        user_trace_dbg("User-only trace turned off -> Restoring old trace level at 0x%llx, ASID %d\n",
            (unsigned long long)env->active_tc.PC, env->CP0_EntryHi & 0xFF);
        qemu_set_log(qemu_loglevel | env->trace_level_before_suspend);
    }
    env->tracing_suspended = false;
    env->user_only_tracing_enabled = false;
    user_trace_dbg("User-mode only tracing disabled at 0x%llx, ASID %d\n",
        (unsigned long long)env->active_tc.PC, ASID);
}
...
static void update_tracing_on_mode_change(CPUMIPSState *env, const char* new_mode)
{
    ...
    if (IN_USERSPACE(env)) {
        ...
        user_trace_dbg("%s -> %s: 0x%llx ASID %d -- switching off tracing \n",
        ...
        if (env->tracing_suspended) {
            qemu_set_log(qemu_loglevel | env->trace_level_before_suspend);
            user_trace_dbg("%s -> %s 0x%llx ASID %d -- switching on tracing\n",
                env->last_mode, new_mode, (unsigned long long)env->active_tc.PC,
                env->CP0_EntryHi & 0xFF);
            env->tracing_suspended = false;
        }
